...
Now can use function {\\tt plotPara} to calculate revelant parameters for ggplot. Modified funcition {\tt equa} can be used for calculating SER10%.
<<eval=FALSE, echo=TRUE>>=
  para = plotPara(fit)
  ## para$pd is data for geom_point and geom_errorbar, para$dose is α and β parameters.
  ggplot(para$pd, aes(x = x, y = st)) +
  geom_function(fun = equa, colour = "black") +
  geom_point(aes(x = x, y = st, fill = treat), shape = 18, size = 3) +
  geom_errorbar(
    aes(ymin = st - sd, ymax = st + sd),
    width = 0.05,
    linewidth = 0.8,
    color = "black",
    position = "identity"
  ...
  scale_y_continuous(
    ...
    minor_breaks = as.numeric(1:10 %o% 10 ^ -(1:10)),
